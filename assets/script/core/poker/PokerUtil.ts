...
export function getHandValueByPokerType(pub: number[], hand: number[], pe: InnerPokerType): HandValue[] {
    ...
    if (lh === 2) {
        ...
        if (totalLen === 6) {
            ...
            for (let i = 0; i < 6; i++) {
                ...
                buffer[0] = nc[idx[0]]; buffer[1] = nc[idx[1]]; buffer[2] = nc[idx[2]]; buffer[3] = nc[idx[3]]; buffer[4] = nc[idx[4]];
    ...
    if (lp === 3) {
        buffer[0] = pub[0]; buffer[1] = pub[1]; buffer[2] = pub[2];
        for (let i = 0; i < lh2; i++) {
            const hIdx = handComb2[i];
            buffer[3] = hand[hIdx[0]]; buffer[4] = hand[hIdx[1]];
            hands.push(HandValue.create(buffer, pe));
        }
        return hands;
    } 
    ...
    if (lp === 4) {
        for (let i = 0; i < 4; i++) {
            ...
            buffer[0] = pub[pIdx[0]]; buffer[1] = pub[pIdx[1]]; buffer[2] = pub[pIdx[2]];
