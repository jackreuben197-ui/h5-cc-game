    ...
    if (roomData.basicInfo.antiCheatConfig) {
        ...
        try {
            ...
            if (seatedConfig.enableCamera && roomData.basicInfo.antiCheatConfig.isOptionalVideoMaskMode) {
                const initialMaskId = seatedConfig.openPowerSaving ? mine.player.videoMaskId || 1 : 0;
                if (initialMaskId != mine.player.videoMaskId && (await TexasVideoMediaHelper.setVideoMask(initialMaskId))) {
                    mine.player.videoMaskId = initialMaskId;
                }
            }
            if (seatedConfig.enableCamera && seatedConfig.canSwitchPowerSaving) {
                mine.player.realShowMaskID = seatedConfig.openPowerSaving
                    ? roomData.basicInfo.antiCheatConfig.getVisibleVideoMaskId(mine.player.videoMaskId || 1)
                    : 0;
                roomData.mine.maskBtnState = mine.player.realShowMaskID > 0 ? ButtonState.ON : ButtonState.OFF;
