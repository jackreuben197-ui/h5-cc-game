import { BRIDGE_MSG_TYPE, normalizeBridgeMsgType, type BridgeAction, type BridgeMsgType } from './actions';
...
export function createBridgeMessage<TPayload>(
    ...
    return {
        action,
        payload,
        msgtype: options.msgtype === undefined ? BRIDGE_MSG_TYPE.H5 : normalizeBridgeMsgType(options.msgtype),
...
export function parseBridgeRaw(raw: string): BridgeMessage | null {
    ...
    const maybeDecoded = schemeHandled.includes('%7B') || schemeHandled.includes('%22') ? decodeURIComponent(schemeHandled) : schemeHandled;
    try {
...
function normalizeBridgeMessage(parsed: PartialBridgeMessage): BridgeMessage | null {
    ...
    return {
        ...
        requestId: typeof parsed.requestId === 'string' ? parsed.requestId : `cocos_${Date.now()}_${Math.random().toString(36).slice(2, 9)}`,
