...
// ─── Bridge 协议类型 ──────────────────────────────────────────────────────
// 单一来源：h5-cc-bridge 仓库的 cc-side 入口（只含 action 常量 + payload 类型，不含 envelope 运行时）。
// 本地维护脚本：`npm run sync:bridge` 把 cc-side 白名单源码复制到 ./bridge。
// 不要在这里重新声明协议字段；要改协议先去 h5-cc-bridge 仓库。
import { H5NavigatePayload, H5ToCocosPayloadMap, CocosToH5PayloadMap as SharedCocosToH5PayloadMap } from './bridge/cc-side';
